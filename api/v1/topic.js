    ...
    ep.after('author', topics.length, function () {
      topics = topics.map(function (topic) {
        return _.pick(topic, ['id', 'author_id', 'tab', 'content', 'title', 'last_reply_at',
          'good', 'top', 'reply_count', 'visit_count', 'create_at', 'author', 'fulcrum', 'tab_img'
        ]);
      });
...
var create = function (req, res, next) {
  ...
  TopicProxy.newAndSave(title, content, tab, req.user.id, fulcrum, tab_img, function (err, topic) {
    if (err) {
      return next(err);
    }
